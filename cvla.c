...
CVLArrayStatusCode cvla_new_nocheck(size_t arr_length, size_t arr_elsz, void* arr, CVLArray** cvla) {
    CVLArray* new_cvla = malloc(sizeof(CVLArray));
...
CVLArrayStatusCode cvla_fpush_nocheck(void* el, CVLACopyFunc cpy_func, CVLArray* cvla) {
    if (cvla->_cvla_sz == cvla->_cvla_length) {
#ifdef __MEM_SENSITIVE__
        CVLArrayStatusCode cvla_expand_status = cvla_expand(cvla);
        if (cvla_expand_status != CVLASuccess) {
...
CVLArrayStatusCode cvla_push_nocheck(void* el, CVLArray* cvla) {
    if (cvla->_cvla_sz == cvla->_cvla_length) {
#ifdef __MEM_SENSITIVE__
        CVLArrayStatusCode cvla_expand_status = cvla_expand(cvla);
        if (cvla_expand_status != CVLASuccess) {
...
CVLArrayStatusCode cvla_fpop_nocheck(void* el, CVLACopyFunc cpy_func, CVLArray* cvla) {
    if (cvla->_cvla_sz == 0) {
        return CVLAZeroSizeError;
    }
...
CVLArrayStatusCode cvla_pop_nocheck(void* el, CVLArray* cvla) {
    if (cvla->_cvla_sz == 0) {
        return CVLAZeroSizeError;
    }
...
CVLArrayStatusCode cvla_fprint_nocheck(FILE* f, CVLAPrintFunc print_func, CVLArray* cvla) {
    print_func(f, cvla->_cvla, cvla->_cvla_sz);
